...
if __name__ == '__main__':
    ...
    with open(endereco, 'r') as arquivo:
        ...
        for data in data_list:
            data['percentual'] = percentual_de_uso(data['espaco_utilizado'], espaco_total_utilizado)
            data['espaco_utilizado'] = data['espaco_utilizado']

    data_list = sorted(data_list, key=lambda x: x['percentual'], reverse=True)
...
"""
    ...
    with open('arquivos/relatorio_gerado.txt', 'w') as arquivo:
        numero_de_elementos = int(input('Digite quantos elementos você quer no relatório: '))
        data_list = data_list[:numero_de_elementos]
        arquivo.writelines(cabecalho)
        for indice, dados in enumerate(data_list):
            arquivo.writelines(f"{indice:<4} {dados['usuario']:14} {dados['espaco_utilizado']:>13} MB {dados['percentual']:>11}%\n")
